...
namespace Lithuaningo.API.Services.Flashcards
{
    ...
    public class FlashcardService : IFlashcardService
    {
        ...
        private const double FlashcardGenerationOverheadFactor = 1.5; // Request 50% more flashcards than needed
        ...
        public async Task<IEnumerable<FlashcardResponse>> GenerateFlashcardsAsync(FlashcardRequest request, int contextSampleSize = 100)
        {
            ...
            try
            {
                ...
                // Save ALL unique generated flashcards to Supabase
                if (allGeneratedUniqueFlashcards.Any())
                {
                    await SaveFlashcardsToSupabaseAsync(allGeneratedUniqueFlashcards, request.UserId);
                }
                else
                {
            ...
            List<string> existingFrontTexts)
        {
            ...
            {
                PrimaryCategory = request.PrimaryCategory,
                Count = (int)Math.Ceiling(request.Count * FlashcardGenerationOverheadFactor), // Use the new factor
                UserId = request.UserId,
                Difficulty = request.Difficulty
            };

            // Retry generation until we have enough flashcards or reach max attempts
            while (flashcards.Count < currentRequest.Count && attemptCount < maxAttempts) // Compare with currentRequest.Count which includes overhead
            {
                attemptCount++;
                ...
                // If we got enough flashcards (including overhead) for this attempt's request, break.
                // The final trimming to original request.Count happens in the calling method.
                if (flashcards.Count >= currentRequest.Count)
                {
                    break;
                }
            }

            // Log if we couldn't generate enough to even meet the overhead request count
            if (flashcards.Count < currentRequest.Count && flashcards.Count < request.Count)
            {
